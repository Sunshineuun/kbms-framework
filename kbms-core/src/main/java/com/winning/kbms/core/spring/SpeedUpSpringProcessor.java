package com.winning.kbms.core.spring;
...
public class SpeedUpSpringProcessor implements BeanFactoryPostProcessor
{
    ...
    /**
     * 需要移除的bean及属性名字 如 bean@property 或 需要替换掉值的bean及属性名字 如 bean@property=123
     * 
    ...
    public void postProcessBeanFactory (ConfigurableListableBeanFactory beanFactory) throws BeansException
    {
        ...
        {
            log.error ("如果需要给Spring加速, bean factory 类型必须是 DefaultListableBeanFactory");
            return;
        }
    ...
    {
        if (noneLazyBeanNames == null || noneLazyBeanNames.length == 0)
        {
            return false;
        }
    ...
    private boolean needRemove (String beanName, BeanDefinition beanDefinition)
    {
        ...
        if (removedClassPatterns == null || removedClassPatterns.length == 0)
        {
            return false;
        }

        if (includeClassPatterns != null)
        {
