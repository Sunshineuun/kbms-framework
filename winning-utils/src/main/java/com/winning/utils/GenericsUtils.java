...
public class GenericsUtils {
	...
	public static Class getSuperClassGenricType(Class clazz, int index) {
		...
		if (index >= params.length || index < 0) {
			throw new RuntimeException("泛型的索引" + (index < 0 ? "不能小于0" : "超出了参数的总数"));
		}
	...
	@SuppressWarnings("rawtypes")
	public static Class getInterfaceGenricType(Class clazz, Class<?> interfaceClass, int index) {
		Type[] genTypes = clazz.getGenericInterfaces();// 得到泛型父类

		// 如果没有实现ParameterizedType接口，即不支持泛型，直接返回Object.class
		for (Type genType : genTypes) {
			if (genType instanceof ParameterizedType && ((ParameterizedType) genType).getRawType() == interfaceClass) {
