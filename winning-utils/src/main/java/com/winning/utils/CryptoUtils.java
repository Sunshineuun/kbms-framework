 ...
 * 
 * 
 * $Id: CryptoUtils.java,v 1.6 2016/05/16 09:00:00 yuanfeng Exp $
 * ================================================================
 */
...
import com.winning.utils.EncodeUtils;

/**
 * 支持SHA-1消息摘要, HMAC-SHA1消息签名 及 DES对称加密、MD5的Util方法集合.
 * 
 * 支持Hex与Base64两种编码方式.
 * 
 */
public class CryptoUtils {
	...
	/**
	 * 生成HMAC-SHA1密钥,返回字节数组. HMAC-SHA1算法(RFC2401)建议最少长度为160位.
	 */
	...
	/**
	 * 生成HMAC-SHA1密钥, 返回Hex编码的结果. HMAC-SHA1算法可使用任意长度的密钥, 默认长度为160位.
	 */
	...
	/**
	 * 生成HMAC-SHA1密钥, 返回Base64编码的结果. HMAC-SHA1算法可使用任意长度的密钥, 默认长度为160位.
	 */
	...
	public static String desEncryptToHex(String input, byte[] keyBytes) {
		byte[] encryptResult = des(input.getBytes(), keyBytes,
				Cipher.ENCRYPT_MODE);
	...
	public static String desDecryptFromHex(String input, byte[] keyBytes) {
		byte[] decryptResult = des(EncodeUtils.hexDecode(input), keyBytes,
				Cipher.DECRYPT_MODE);
